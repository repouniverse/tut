        ...
        'codalu',
         ...
         'id','talleres_id','talleresdet_id','fechaprog','codtra',
           'finicio','ftermino','fingreso','codaula',    
             'fechaprog1','finicio1','ftermino1','numerocita','flujo_id'    
           ], 'safe'],
...
//var_dump($this->asistio);
     ...
     public function searchByIds($params,$ids)
    {
        $query = StaVwCitas::find();
...
//var_dump($this->asistio);
   ...
   public function searchByPsicoToday($codfac)
    ...
    public function searchByDay($codfac,$fecha)
    {
        $formato= \common\helpers\timeHelper::formatMysqlDate();
       $formatoTime= \common\helpers\timeHelper::formatMysqlDateTime();
      $codperiodo=  staModule::getCurrentPeriod();
      $carbon= Carbon::createFromFormat(
        ...
        $dataProvider = new ActiveDataProvider([
         ...
         $query->andWhere(['codfac'=> $codfac])-> 
             ...
             'between',
             'fechaprog',
             $carbon->endOfDay()->subDay(1)->format(
               $formatoTime
               ),
            $carbon->copy()->endOfDay()->addDay(1)->format(
               $formatoTime
               )
                        ])->orderBy('fechaprog asc'); 
         //echo date('Y-m-d H:i:s');die();
